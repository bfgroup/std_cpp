...
* Already in wide use.
* Experiential knowledge of behavior as a _vendor argument_.
* Have a meaningful, but brief, long form name.
...
WARNING: This is also a currently limited and incomplete specification.
...
program executable. And `arguments[1..N)` are either either
options or names of source files.
...
_Experimental options_ follow the same syntax as _core options_.
