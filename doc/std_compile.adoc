...
existing compiler frontend tool options for transforming source code to
...
that SG15 could meet and approve changes to those _Standard Documents_
as needed.
...
My research, with regards to reading the the standard text, is that even though
the standard does tread a fine line to avoid specifics about the interaction
...
or transformed translation units to link into a program.

Options, and their values, can be "link incompatible". Wherein translation
units transformed with differing options or values can not be linked together
into a program.
