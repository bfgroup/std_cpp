...
For instance::
We specify the `{lopt}address_model` core option. It would not be allowed for
a vendor to specify an `{lopt}v:xyz:m64` (to specify 64 bit address model) option
...
use `{sopt}` as the prefix, and hence `{lopt}` for the long form, for these reasons:
...
start with the `{sopt}` or `{lopt}` option prefix.

===== Long & Short Options

Arguments that start with `{lopt}` are _long options_. And arguments that start
with a single `{sopt}` are _short options_. _Long options_ are expected to be
...
[subs="normal"]
....
{lopt}__name__[=__value__]
{lopt}__name__ [__value__]
{sopt}__n__ [__value__]
....
...
[subs="normal"]
....
{lopt}__vendor-key__:__name__[=__value__]
{lopt}__vendor-key__:__name__ [__value__]
{sopt}__vendor-key__:__n__ [__value__]
....
